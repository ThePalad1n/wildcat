...
class BST:
    ...
    # Main parse method to parse the input expression
    def parse(self):
    ...
    def _parse_term(self):
        left = self._parse_factor()
        while self.index < len(self.tokens) and self.tokens[self.index].token_type in ["GREATERTHAN", "LESSTHAN"]:
            op = self.tokens[self.index]
            self.index += 1
            right = self._parse_factor()
            node = Node(op)
    ...
    def _parse_term(self):
        left = self._parse_factor()
        while self.index < len(self.tokens) and self.tokens[self.index].token_type in ["GREATERORIS", "LESSERORIS"]:
            op = self.tokens[self.index]
            self.index += 1
            right = self._parse_factor()
            node = Node(op)
    ...
    def _parse_term(self):
        left = self._parse_factor()
        while self.index < len(self.tokens) and self.tokens[self.index].token_type in ["IS", "NOT"]:
            op = self.tokens[self.index]
            self.index += 1
            right = self._parse_factor()
            node = Node(op)
    ...
    def _parse_term(self):
        left = self._parse_factor()
        while self.index < len(self.tokens) and self.tokens[self.index].token_type in ["MODULO"]:
            op = self.tokens[self.index]
            self.index += 1
            right = self._parse_factor()
            node = Node(op)
    ...
    # Helper method for deleting a node with a specific value
    ...
    # Traversal method for postorder
    ...
    # Traversal method for value order
    ...
    # Method to get the depth of a specific node in the BST
...
# Function to evaluate the result of the parsed expression
def evaluate(node):
    if node.left is None and node.right is None:
        ...
        if node.data.token_type == "PLUS":
            ...
            return left / right
        elif node.data.token_type == "GREATERTHAN":
            if left > right:
                return True
            elif left < right:
                return False
                ...
                return False
        elif node.data.token_type == "LESSTHAN":
            if left < right:
                return True
            elif left > right:
                return False
        ...
        elif node.data.token_type == "GREATERORIS":
            if left >= right:
                return True
            elif left < right:
                return False
        elif node.data.token_type == "LESSERORIS":
            if left <= right:
                return True
            elif left > right:
                return False

        elif node.data.token_type == "IS":
            if left == right:
                return True
            elif left != right:
                return False

        elif node.data.token_type == "NOT":
            if left == right:
                return False
            elif left != right:
                return True

        elif node.data.token_type == "MODULO":
                return left % right

        else:
            raise ValueError(f"Unknown operator {node.data.token_type}")
#add modulo
# Define the categorization of language elements
categorized ={
'literals': [r'\d+|\'.*?\'|\".*?\"'],
'operators': [r'\plus|\minus|\times|\divide|\modulo'],
...
'comparative_operators': [r'\is|\not|\greater\sthan|\less\sthan|\greater\sor\sis|\lesser\sor\sis']
}
#add modulo
# Define token types
TOKEN_TYPES = [
    ...
    ("GREATERORIS", r"greater\sor\sis"),
    ("LESSERORIS", r"lesser\sor\sis"),
]
